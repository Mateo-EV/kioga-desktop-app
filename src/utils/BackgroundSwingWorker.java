...
public class BackgroundSwingWorker extends SwingWorker<Object, Void> {
    ...
    public BackgroundSwingWorker(String[] endpoints, String[] methods,
        Map<String, Object>[] data, ApiClient.onResponse callback,
        boolean[] expectList) {
    ...
    public BackgroundSwingWorker(String endpoint, String method,
        Map<String, Object> data, ApiClient.onResponse callback,
        boolean expectList) {
    ...
    protected Object doInBackground() throws Exception {
        try {
            if (singleRequest) {
                if (expectList[0]) {
                    return ApiClient.sendRequestForList(endpoints[0], methods[0],
                        data[0]);
                } else {
                    return ApiClient.sendRequest(endpoints[0], methods[0],
                        data[0]);
                }
            }
            ...
            for (int i = 0; i < numRequests; i++) {
                ...
                new Thread(() -> {
                    try {
                        Object result;
                        if (expectList[index]) {
                            result = ApiClient.sendRequestForList(
                                endpoints[index], methods[index],
                                data == null ? null : data[index]);
                        } else {
                            result = ApiClient.sendRequest(endpoints[index],
                                methods[index],
                                data == null ? null : data[index]);
                        }
    ...
    protected void done() {
        try {
            Object response = get();
            callback.onSuccess(new ApiClient.ApiResponse(
                ApiClient.ResponseType.SUCCESS, "Request successful", response));
        } catch (Exception e) {
            if (e.getCause() instanceof ApiClient.ApiException) {
                ApiClient.ApiException apiException = (ApiClient.ApiException) e.getCause();
                callback.onError(new ApiClient.ApiResponse(
                    ApiClient.ResponseType.ERROR, apiException.getErrorBody()));
            } else {
                callback.onError(new ApiClient.ApiResponse(
                    ApiClient.ResponseType.ERROR, "Algo malo ocurrió"));
            }
            e.printStackTrace();
        }
    }
