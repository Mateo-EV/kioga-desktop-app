...
public class EditCategoryForm extends javax.swing.JPanel {
    ...
    private Category categoryLoaded = new Category();

    public EditCategoryForm(int id) {
        initComponents();
        categoryLoaded.setId(id);
        init();
    }
    ...
    private void loadImage() {
        ...
        if (imageCache.containsKey(categoryLoaded.getImage())) {
            pic.setImage(imageCache.get(categoryLoaded.getImage()));
        } else {
            try {
                URL imageUrl = new URL(categoryLoaded.getImage());
                BufferedImage image = ImageIO.read(imageUrl);
                ImageIcon icon = new ImageIcon(TableImage.resizeImage(image, 60,
                    60));
                imageCache.put(categoryLoaded.getImage(), icon);
                pic.setImage(icon);
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private void init() {
        ...
        CategoryController.getInstance().findById(categoryLoaded.getId(),
            new ApiClient.onResponse() {
            @Override
            public void onSuccess(ApiClient.ApiResponse apiResponse) {
                categoryLoaded = (Category) apiResponse.getData();
                txtName.setText(categoryLoaded.getName());
                loadImage();
                stopLoadingState();
            }
        ...
        saveBtn.addActionListener((e) -> {
            ...
            category.setId(categoryLoaded.getId());
